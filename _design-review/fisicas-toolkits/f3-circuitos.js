...
(function () {
  ...
  /* ============================================================
     SIM 3 — Serie, paralelo y mixto  (NUEVA)
     ============================================================ */
  SimKit.registerSim({
    tab: "f3",
    section: "Circuitos",
    id: "redes",
    title: "Serie, paralelo y mixto",
    refs: "Resumen p.16 · Práctica g.2 (2.14, 2.16)",
    blurb: "Tres topologías precableadas de R₁, R₂, R₃ con una fuente V. R_eq se arma paso a paso; cada " +
      "rama muestra su corriente (flecha ∝ |i|) y su caída de tensión. Arrastrá el multímetro sobre " +
      "cualquier rama para medir su i y su ΔV.",
    formula: "serie: R_eq=ΣR (I común)   ·   paralelo: 1/R_eq=Σ1/R (V común)   ·   mixto: R₁+(R₂∥R₃)   ·   V=IR",
    stageHeight: 360,
    controls: [
      { type: "segment", key: "topo", label: "Topología", value: "serie", options: [
        ["serie", "Serie"], ["par", "Paralelo"], ["mix", "Mixto"]] },
      { type: "range", key: "V", label: "fuente V", min: 1, max: 24, step: 1, value: 12, unit: "V", digits: 0 },
    ...
    readouts: [
      { key: "req", label: "R_eq", unit: "Ω", digits: 2 },
      { key: "it", label: "I total", unit: "A", digits: 2 },
      { key: "im", label: "multímetro i", unit: "A", digits: 3 },
      { key: "vm", label: "multímetro ΔV", unit: "V", digits: 2 },
    ],
    create: function (env) {
      var probe = { x: 0, y: 0 };
      var needInit = true;
      ...
      function solve() {
        var p = env.params, R1 = p.R1, R2 = p.R2, R3 = p.R3, V = p.V;
        if (p.topo === "serie") {
          var Req = R1 + R2 + R3, I = V / Req;
          return { Req: Req, It: I, br: [
            { name: "R₁", i: I, dv: I * R1 }, { name: "R₂", i: I, dv: I * R2 }, { name: "R₃", i: I, dv: I * R3 }],
            steps: [
              "R_eq = R₁+R₂+R₃",
              "    = " + R1 + "+" + R2 + "+" + R3 + " = " + Req + " Ω",
              "I = V/R_eq = " + V + "/" + Req + " = " + I.toFixed(2) + " A",
              "misma I en las 3;  ΔVₖ = I·Rₖ"] };
        } else if (p.topo === "par") {
          var Req2 = 1 / (1 / R1 + 1 / R2 + 1 / R3);
          var i1 = V / R1, i2 = V / R2, i3 = V / R3, It = i1 + i2 + i3;
          return { Req: Req2, It: It, br: [
            { name: "R₁", i: i1, dv: V }, { name: "R₂", i: i2, dv: V }, { name: "R₃", i: i3, dv: V }],
            steps: [
              "1/R_eq = 1/R₁+1/R₂+1/R₃",
              "R_eq = " + Req2.toFixed(2) + " Ω",
              "misma V=" + V + " en las 3",
              "iₖ = V/Rₖ;  I = Σiₖ = " + It.toFixed(2) + " A"] };
        } else {
          var R23 = R2 * R3 / (R2 + R3), Req3 = R1 + R23;
          var I1 = V / Req3, Vp = I1 * R23, i2b = Vp / R2, i3b = Vp / R3;
          return { Req: Req3, It: I1, br: [
            { name: "R₁", i: I1, dv: I1 * R1 }, { name: "R₂", i: i2b, dv: Vp }, { name: "R₃", i: i3b, dv: Vp }],
            steps: [
              "R₂∥R₃ = R₂R₃/(R₂+R₃) = " + R23.toFixed(2) + " Ω",
              "R_eq = R₁ + R₂∥R₃ = " + Req3.toFixed(2) + " Ω",
              "I₁ = V/R_eq = " + I1.toFixed(2) + " A",
              "V_par = I₁·(R₂∥R₃) = " + Vp.toFixed(2) + " V"] };
        }
      }

      function reset() { needInit = true; }
      function initProbe() { probe.x = (env.W || 600) * 0.32; probe.y = (env.H || 360) * 0.5; needInit = false; }
      ...
      function iArrow(ctx, x0, y0, x1, y1, i, col) {
        var mag = clamp(Math.abs(i) * 3.2, 1.4, 7);
        drawArrow(ctx, x0, y0, x1, y1, i >= 0 ? col.teal : col.red, mag, 8);
      }

      function draw(ctx, W, H) {
        if (needInit && W > 0) initProbe();
        var col = env.colors, p = env.params, s = solve();
        var loopL = 56, loopR = W * 0.60, loopT = H * 0.24, loopB = H * 0.80;
        var ym = (loopT + loopB) / 2, bx = loopL;
        var mids = [];

        ctx.strokeStyle = col.dim; ctx.lineWidth = 2;

        if (p.topo === "serie") {
          // lazo con 3 resistores en el riel superior
          ctx.beginPath();
          ctx.moveTo(loopL, loopT); ctx.lineTo(loopR, loopT);   // riel superior
          ctx.lineTo(loopR, loopB); ctx.lineTo(loopL, loopB);   // derecho + inferior
          ctx.moveTo(loopL, loopT); ctx.lineTo(loopL, ym - 12); // batería arriba
          ctx.moveTo(loopL, ym + 12); ctx.lineTo(loopL, loopB); // batería abajo
          ctx.stroke();
          var fr = [1 / 6, 3 / 6, 5 / 6];
          for (var a = 0; a < 3; a++) {
            var xk = loopL + fr[a] * (loopR - loopL);
            resistorSym(ctx, xk, loopT, true, col);
            mids.push({ mx: xk, my: loopT, i: s.br[a].i, dv: s.br[a].dv, name: s.br[a].name });
            ctx.fillStyle = col.amber; mono(ctx, 11);
            ctx.fillText(s.br[a].name + "=" + p["R" + (a + 1)] + "Ω", xk - 16, loopT - 12);
            ctx.fillStyle = col.dim; mono(ctx, 10);
            ctx.fillText("ΔV=" + s.br[a].dv.toFixed(1), xk - 16, loopT + 22);
          }
          // corriente única (loop) — retorno por el riel inferior (→ batería −)
          iArrow(ctx, loopL + 58, loopB - 4, loopL + 18, loopB - 4, s.It, col);
          ctx.fillStyle = col.teal; mono(ctx, 11); ctx.fillText("I", loopL + 62, loopB - 6);
        } else if (p.topo === "par") {
          // dos rieles + 3 ramas verticales
          ctx.beginPath();
          ctx.moveTo(loopL, loopT); ctx.lineTo(loopR, loopT);   // riel + (superior)
          ctx.moveTo(loopL, loopB); ctx.lineTo(loopR, loopB);   // riel − (inferior)
          ctx.moveTo(loopL, loopT); ctx.lineTo(loopL, ym - 12);
          ctx.moveTo(loopL, ym + 12); ctx.lineTo(loopL, loopB);
          ctx.stroke();
          var frp = [0.34, 0.58, 0.82];
          for (var b = 0; b < 3; b++) {
            var xb = loopL + frp[b] * (loopR - loopL);
            ctx.strokeStyle = col.dim; ctx.lineWidth = 2;
            ctx.beginPath(); ctx.moveTo(xb, loopT); ctx.lineTo(xb, loopB); ctx.stroke();
            resistorSym(ctx, xb, ym, false, col);
            mids.push({ mx: xb, my: ym, i: s.br[b].i, dv: s.br[b].dv, name: s.br[b].name });
            iArrow(ctx, xb, loopT + 14, xb, ym - 22, s.br[b].i, col);
            ctx.fillStyle = col.amber; mono(ctx, 11);
            ctx.fillText(s.br[b].name + "=" + p["R" + (b + 1)] + "Ω", xb + 10, ym);
            ctx.fillStyle = col.dim; mono(ctx, 10);
            ctx.fillText("i=" + s.br[b].i.toFixed(2), xb + 10, ym + 16);
          }
          ctx.fillStyle = col.blue; mono(ctx, 11);
          ctx.fillText("V=" + p.V + "V (común)", loopL + 6, loopT - 10);
        } else {
          // mixto: R1 serie en riel superior, luego R2∥R3
          var xB = loopL + 0.48 * (loopR - loopL);
          ctx.strokeStyle = col.dim; ctx.lineWidth = 2;
          ctx.beginPath();
          ctx.moveTo(loopL, loopT); ctx.lineTo(loopR, loopT);   // riel superior completo
          ctx.moveTo(loopL, loopB); ctx.lineTo(loopR, loopB);   // riel inferior
          ctx.moveTo(xB, loopT); ctx.lineTo(xB, loopB);         // rama R2
          ctx.moveTo(loopR, loopT); ctx.lineTo(loopR, loopB);   // rama R3
          ctx.moveTo(loopL, loopT); ctx.lineTo(loopL, ym - 12);
          ctx.moveTo(loopL, ym + 12); ctx.lineTo(loopL, loopB);
          ctx.stroke();
          var xR1 = (loopL + xB) / 2;
          resistorSym(ctx, xR1, loopT, true, col);
          resistorSym(ctx, xB, ym, false, col);
          resistorSym(ctx, loopR, ym, false, col);
          mids.push({ mx: xR1, my: loopT, i: s.br[0].i, dv: s.br[0].dv, name: "R₁" });
          mids.push({ mx: xB, my: ym, i: s.br[1].i, dv: s.br[1].dv, name: "R₂" });
          mids.push({ mx: loopR, my: ym, i: s.br[2].i, dv: s.br[2].dv, name: "R₃" });
          iArrow(ctx, loopL + 12, loopT - 0, xR1 - 24, loopT - 0, s.br[0].i, col);
          iArrow(ctx, xB, loopT + 14, xB, ym - 22, s.br[1].i, col);
          iArrow(ctx, loopR, loopT + 14, loopR, ym - 22, s.br[2].i, col);
          ctx.fillStyle = col.amber; mono(ctx, 11);
          ctx.fillText("R₁=" + p.R1 + "Ω", xR1 - 16, loopT - 12);
          ctx.fillText("R₂=" + p.R2 + "Ω", xB + 10, ym);
          ctx.fillText("R₃=" + p.R3 + "Ω", loopR + 10, ym);
          ctx.fillStyle = col.dim; mono(ctx, 10);
          ctx.fillText("I₁=" + s.br[0].i.toFixed(2), xR1 - 16, loopT + 22);
        }

        // batería (izquierda, vertical)
        ctx.strokeStyle = col.coral; ctx.lineWidth = 3;
        ctx.beginPath(); ctx.moveTo(bx - 11, ym - 12); ctx.lineTo(bx + 11, ym - 12); ctx.stroke();
        ctx.strokeStyle = col.blue; ctx.lineWidth = 6;
        ctx.beginPath(); ctx.moveTo(bx - 7, ym + 12); ctx.lineTo(bx + 7, ym + 12); ctx.stroke();
        ctx.fillStyle = col.dim; mono(ctx, 11);
        ctx.fillText(p.V + "V", bx + 14, ym + 4);

        // ===== desglose paso a paso (columna derecha) =====
        var tx = W * 0.64, ty = 34;
        ctx.fillStyle = col.teal; mono(ctx, 11);
        ctx.fillText("R_eq paso a paso", tx, ty);
        ctx.fillStyle = col.text; mono(ctx, 11);
        for (var t = 0; t < s.steps.length; t++) ctx.fillText(s.steps[t], tx, ty + 22 + t * 20);

        // ===== multímetro: rama más cercana =====
        var best = mids[0], bd = 1e9;
        for (var mi = 0; mi < mids.length; mi++) {
          var dd = Math.hypot(probe.x - mids[mi].mx, probe.y - mids[mi].my);
          if (dd < bd) { bd = dd; best = mids[mi]; }
        }
        ctx.strokeStyle = "rgba(255,255,255,0.5)"; ctx.setLineDash([3, 3]); ctx.lineWidth = 1;
        ctx.beginPath(); ctx.arc(best.mx, best.my, 22, 0, TAU); ctx.stroke(); ctx.setLineDash([]);
        ctx.fillStyle = col.text; mono(ctx, 11);
        ctx.fillText(best.name + ": i=" + best.i.toFixed(3) + " A", probe.x + 14, probe.y - 6);
        ctx.fillStyle = col.dim;
        ctx.fillText("ΔV=" + best.dv.toFixed(2) + " V", probe.x + 14, probe.y + 10);

        env.setReadout("req", s.Req);
        env.setReadout("it", s.It);
        env.setReadout("im", best.i);
        env.setReadout("vm", best.dv);
      }
  ...
  /* ============================================================
     SIM 4 — Kirchhoff (2 mallas)  (PORT de f3.js + enriquecido)
     ============================================================ */
  SimKit.registerSim({
    tab: "f3",
    section: "Circuitos",
