...
function Recommendations({
  ...
  // Una fila densa por electiva (RecRow compartida con el Combinador). La señal
  // "alarga / no alarga" NO va como chip: ya la da el grupo que contiene la fila.
    ...
    <RecRow
      ...
      signals={
        r.conflict ? (
          ...
          <>
            <RecSig tone="when" title={cuatriName(cuatriAt(start, r.landingIdx))}>
              {cuatriLabel(cuatriAt(start, r.landingIdx))}
            </RecSig>
            {r.newDays > 0 && (
              <RecSig
                tone="soft"
                title={`Suma ${r.newDays} ${r.newDays === 1 ? "día" : "días"} de campus a ese cuatrimestre`}
              >
                +{r.newDays}d
              </RecSig>
            )}
