...
- [[Maven Module Structure]] — la estructura multi-módulo (`pom-root` + webapp/services/persistence/…).
- [[Spring Configuration]] — `WebConfig`, `DispatcherServlet` y el `ViewResolver`.
- [[Dependency Injection]] — `@Autowired`, estereotipos y resolución con `@Qualifier`/`@Primary`.
- [[Controllers & Validation]] — los `@Controller` que arma esta clase.
